...
def LU_blocked( A, bn = 500 ):   # John Cole: LU FACTORIZATION MEASUREMENT AND OPTIMIZATION
	# maxBlock_n ..... right looking algorithm maximal block size
	(n,m) = A.shape
	A = A*1    # just not to change the original from otside of the function
	if bn == 1:
		return LU(A)
	...
	for k in range(steps):
		if k == steps-1:
			...
			P[bn*k:,:] = np.dot(Pk, P[bn*k:,:])
			L[bn*k:,:bn*k] = np.dot(Pk, L[bn*k:,:bn*k])
			return L, A, P
		...
		# (Cole 4)
		A[bn*k:,bn*(k+1):], L[bn*k:,:bn*k] = np.dot(Pk, A[bn*k:,bn*(k+1):]), np.dot(Pk, L[bn*k:,:bn*k])
		P[bn*k:,:] = np.dot(Pk, P[bn*k:,:])
		# solve L11*A12 = A12
...
def solveLinearSystem( A, b, blockN = 500):  # solve linear system A*x = b, return x
	...
	pb = np.dot(P, b)     # we want to solve (P*A) * x = (P*b), because we know P*A=L*U
